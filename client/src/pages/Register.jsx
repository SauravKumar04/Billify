...
const Register = () => {
  ...
  const onSubmit = async (values) => {
    try {
      ...
      toast.success(data.message || "Welcome to Billify");
      navigate("/dashboard");
    } catch (error) {
    ...
    <main className="flex min-h-screen items-center justify-center px-4 py-8">
      <div className="w-full max-w-md">
        ...
        <div className="shell-card p-6 sm:p-7">
          <h2 className="mb-5 text-xl font-semibold tracking-tight text-[color:var(--ink)]">Register</h2>
          <form className="space-y-3" onSubmit={handleSubmit(onSubmit)}>
            <div>
              <label className="field-label">Full Name</label>
              <div className="relative">
                <RiUser3Line className="pointer-events-none absolute left-3 top-3 text-[color:var(--muted)]" />
              <input
          ...
          <p className="mt-4 text-sm text-[color:var(--muted)]">
            Already have an account? <Link to="/login" className="font-medium text-[color:var(--accent)] underline decoration-[color:var(--accent)] underline-offset-4">Login</Link>
          </p>
        </div>
      </div>
    </main>
