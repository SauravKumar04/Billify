...
const drawHeader = (doc, invoice, user) => {
  ...
  doc
    ...
    .fontSize(10)
    .text("Invoice No", right - 160, 42, { align: "right" });
  doc
    .fontSize(12)
    .fillColor(COLORS.accent)
    .text(invoice.invoiceNumber, right - 160, 58, { align: "right" });
...
const drawParties = (doc, invoice, user) => {
  ...
  doc.text(user.gstin ? `GSTIN: ${user.gstin}` : "", 62, top + 68, {
    width: 210,
  });
...
const drawTotals = (doc, invoice, y, user) => {
  ...
  doc
    .fontSize(12)
    .fillColor(COLORS.ink)
    .text(formatCurrency(invoice.total), 470, row);
  ...
  doc.text(
    `Account Name: ${user.bankDetails?.accountName || ""}`,
    62,
    row + 30,
  );
  doc.text(
  ...
  if (clientUrl && invoice.publicToken) {
    ...
    doc
      .fontSize(10)
      .fillColor(COLORS.muted)
      .text("Client payment link", 62, row + 78);
    doc
      .fontSize(10)
      .fillColor(COLORS.accent)
      .text(link, 180, row + 78, { link, underline: true });
  }
  ...
  if (invoice.notes) {
    ...
    doc
      .fillColor(COLORS.ink)
      .text(invoice.notes, 110, row + 96, { width: 430 });
  }
